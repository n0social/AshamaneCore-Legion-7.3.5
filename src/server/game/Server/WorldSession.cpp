...
bool WorldSessionFilter::Process(WorldPacket* packet)
{
    ClientOpcodeHandler const* opHandle = opcodeTable[static_cast<OpcodeClient>(packet->GetOpcode())];
    if (!opHandle)
        return true; // Unknown opcode - pass through so WorldSession::Update logs the STATUS_UNHANDLED error
...
bool WorldSession::Update(uint32 diff, PacketFilter& updater)
{
    ...
    while (m_Socket[CONNECTION_TYPE_REALM] && _recvQueue.next(packet, updater))
    {
        ...
        if (!opHandle)
        {
            TC_LOG_ERROR("network.opcode", "Received unknown opcode %u from %s", packet->GetOpcode(), GetPlayerInfo().c_str());
            if (deletePacket)
                delete packet;
            deletePacket = true;
            continue;
        }
        // --- CRASH DIAGNOSTIC: log every dispatched opcode ---
        { std::string _on=GetOpcodeNameForLogging(static_cast<OpcodeClient>(packet->GetOpcode())); FILE* _df=fopen("dbg_dispatch.txt","a"); if(_df){fprintf(_df,"Dispatch op=%s status=%d\n",_on.c_str(),(int)opHandle->Status);fflush(_df);fclose(_df);} }
        try
        {
            switch (opHandle->Status)
            {
                case STATUS_LOGGEDIN:
                    ...
                    {
                        if (FILE* dbgFile = fopen("C:\\Users\\donav\\Desktop\\Legion+repack+privateServer\\AshamaneCore-master\\bin\\last_authed_opcode.txt", "w")) { fprintf(dbgFile, "LOGGEDIN: %s\n", GetOpcodeNameForLogging(static_cast<OpcodeClient>(packet->GetOpcode())).c_str()); fclose(dbgFile); }
                ...
                case STATUS_LOGGEDIN_OR_RECENTLY_LOGGOUT:
                    ...
                    else if (AntiDOS.EvaluateOpcode(*packet, currentTime))
                    {
                        ...
                        if (FILE* dbgFile = fopen("C:\\Users\\donav\\Desktop\\Legion+repack+privateServer\\AshamaneCore-master\\bin\\last_authed_opcode.txt", "w")) { fprintf(dbgFile, "LOGGEDIN_OR_RECENTLY_LOGOUT: %s\n", GetOpcodeNameForLogging(static_cast<OpcodeClient>(packet->GetOpcode())).c_str()); fclose(dbgFile); }
                        sScriptMgr->OnPacketReceive(this, *packet);
                        opHandle->Call(this, *packet);
                    }
                    break;
                ...
                case STATUS_AUTHED:
                    ...
                    {
                        TC_LOG_INFO("network", "AUTHED opcode dispatching: %s from %s", GetOpcodeNameForLogging(static_cast<OpcodeClient>(packet->GetOpcode())).c_str(), GetPlayerInfo().c_str());
                        // Write to raw file that survives process crash (log buffer not flushed on crash)
                        {
                            std::string opName = GetOpcodeNameForLogging(static_cast<OpcodeClient>(packet->GetOpcode()));
                            if (FILE* dbgFile = fopen("C:\\Users\\donav\\Desktop\\Legion+repack+privateServer\\AshamaneCore-master\\bin\\last_authed_opcode.txt", "w"))
                            {
                                fprintf(dbgFile, "Last STATUS_AUTHED opcode before crash: %s\n", opName.c_str());
                                fclose(dbgFile);
                            }
                        }
                        sScriptMgr->OnPacketReceive(this, *packet);
...
{
#define ISC_LOG(step) do { FILE* _ff=fopen("dbg_isc.txt","a"); if(_ff){fprintf(_ff,"ISC %d\n",step);fflush(_ff);fclose(_ff);} } while(0)
    ISC_LOG(0);
