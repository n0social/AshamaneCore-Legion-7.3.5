...
{
    // ULTRA-EARLY DIAGNOSTIC
    { FILE* _df=fopen("dbg_start.txt","a"); if(_df){fprintf(_df,"Start type=%d ip=%s\n",(int)_type,GetRemoteIpAddress().to_string().c_str());fflush(_df);fclose(_df);} }
...
void WorldSocket::ReadHandler()
{
    ...
    // DIAGNOSTIC
    { FILE* _df=fopen("dbg_rh.txt","a"); if(_df){fprintf(_df,"ReadHandler type=%d authed=%d\n",(int)_type,(int)_authed);fflush(_df);fclose(_df);} }
...
WorldSocket::ReadDataHandlerResult WorldSocket::ReadDataHandler()
{
    ...
    // CRASH DIAGNOSTIC: log every opcode that arrives on IO thread
    { FILE* _df=fopen("dbg_rdh.txt","a"); if(_df){fprintf(_df,"RDH op=%u type=%d auth=%d sess=%p\n",(unsigned)opcode,(int)_type,(int)_authed,(void*)_worldSession);fflush(_df);fclose(_df);} }
...
{
    { FILE* _df=fopen("dbg_rbac.txt","a"); if(_df){fprintf(_df,"RBAC sess=%p\n",(void*)(_worldSession?_worldSession->GetRBACData():nullptr));fflush(_df);fclose(_df);} }
