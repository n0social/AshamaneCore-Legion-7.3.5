...
{
    if (FILE* dbgF = fopen("C:\\Users\\donav\\Desktop\\Legion+repack+privateServer\\AshamaneCore-master\\bin\\last_authed_opcode.txt", "a")) { fprintf(dbgF, ">> ENTER HandleDBQueryBulk hash=%u\n", dbQuery.TableHash); fclose(dbgF); }
...
{
    if (FILE* dbgF = fopen("C:\\Users\\donav\\Desktop\\Legion+repack+privateServer\\AshamaneCore-master\\bin\\last_authed_opcode.txt", "a")) { fprintf(dbgF, ">> ENTER HandleHotfixRequest count=%zu\n", hotfixQuery.Hotfixes.size()); fclose(dbgF); }
